...
| 核心分工 | 人负责决策，AI负责执行 |
| 本版更新 | GitHub SSH修复、标准tmux会话、AI执行SOP、窗口分工、目录文件说明、P4官方资料核对、OpenClaw CLI安装记录、P4-A本地沙盒链路验证、A1-A6执行结果、A7收尾记录、P4-A收尾评估、P4-B dry-run方案与脚本、P4-C桥接预览脚本、P5-A入口选择策略、P5-B Telegram凭证授权dry-run、P5-C最小channel配置草案、P5-D真实配置审批清单、P5-E入口转向个人飞书、P5-F个人飞书凭证授权dry-run、P5-G最小飞书channel配置草案、P5-H真实个人飞书配置审批清单、P5-I真实配置执行预备检查、P5-J个人飞书真实接入、P5-K本地Codex中转模型验证与LaunchAgent常驻、P5-L运行与回滚SOP、P5-MNO正式部署合并验收、P7日常运维最终验收、Word手册导出记录 |
| 更新日期 | 2026-06-25 |
...
- P5-G 最小个人飞书 channel 配置草案已准备：只生成 patch dry-run、preflight、验收、probe、gateway 启动和回滚命令预览，不执行
- P5-H 真实个人飞书 channel 配置审批清单已准备：明确凭证输入、个人 open_id、patch 文件创建、dry-run、真实写入、provider probe、gateway、消息测试和回滚门禁
- P5-I 真实个人飞书 channel 配置执行预备检查已完成：只读确认 OpenClaw config 有效、gateway 未运行、端口空闲、Feishu channel 未配置、飞书 env 未设置、patch 文件不存在
- P5-J 个人飞书真实接入已完成：复用既有个人飞书机器人，采用 bot 身份、WebSocket、个人 DM allowlist，群聊保持 disabled，不复用公司 `cc-connect` / Feishu
- P5-K 本地 Codex 中转模型链路已对齐并通过实测：本地 Codex API 使用 `xbai` 中转配置，OpenClaw 对齐 `xbai/gpt-5.5`、`openai-responses` 和 `https://api.xbai.top/v1`，不直连官方 OpenAI endpoint
- 个人飞书 DM 实测已通过：测试消息触发模型请求，provider 日志显示 `provider=xbai`、`api=openai-responses`、`model=gpt-5.5`，请求返回 HTTP `200`，飞书 dispatch 完成 `replies=1`
- Gateway 已从临时前台运行切换为 macOS LaunchAgent 常驻：服务已加载运行，RPC 可达，端口 `18789` 正在 loopback 监听，Feishu channel probe 通过
- P5-L 部署收尾已完成：新增 `docs/OPENCLAW_GATEWAY_SOP.md`，完成 Git diff / secret 扫描、只读健康检查、README / AGENTS 阶段校准和运行/回滚边界记录
- P5-MNO 合并验收已完成：短周期入口观察、真实任务边界、任务转发规则和 P5 正式验收合并收口；新增 `docs/DAILY_OPERATIONS_SOP.md`
- P6 NAS 知识库接入只保留位置：暂不挂载 NAS、不配置路径、不写凭据、不启用检索或记忆迁移
- P7 日常运维最终验收已完成：按 `docs/DAILY_OPERATIONS_SOP.md` 完成 gateway、Feishu、模型路由、Git、密钥边界和记录一致性检查
- 当前进入常态日常运维：按每日健康检查、入口规则、任务转发、日志/Git、安全边界、故障处理和回滚策略执行
- Word 手册已按当前 Markdown 同步版导出为可下载版本；导出动作只同步文档，不写入任何真实 secret
- 后续不得新增入口、启用群聊、扩大 allowlist、改模型路由或复用公司通道，除非再次获得用户明确确认
### 后续路线
...
已完成：P3 AI 执行规范固化
↓
已完成：P5-J/P5-K 个人飞书最小真实入口、Codex 中转模型验证与 LaunchAgent 常驻
↓
已完成：P5-L Git 变更审查、运行观测与回滚 SOP
↓
已完成：P5-MNO 入口观察、任务转发边界与 P5 正式验收
↓
保留：P6 NAS 知识库接入位置（暂不执行）
↓
已完成：P7 日常运维最终验收
↓
当前：常态日常运维
↓
后续：按需进入 P6 NAS 知识库接入
```
...
| P5 | OpenClaw 正式部署 | 接入个人飞书消息入口；完成 `xbai` 中转模型链路；建立任务转发边界、回滚 SOP 和正式验收记录。 |
| P6 | NAS 知识库接入 | 暂时只保留位置；后续单独确认后再定义资料分类、路径规范、备份策略和检索方式。 |
| P7 | 日常运维 SOP | 形成每日健康检查、入口规则、任务转发、日志/Git、安全边界、故障处理、回滚和周期维护流程，并完成最终只读健康验收。 |
...
### P5-G 最小个人飞书 channel 配置草案（2026-06-24）

目标：在不执行真实配置的前提下，把未来个人飞书 channel 的最小配置拆成可审阅的 preflight、patch dry-run、真实配置候选、验收、probe、gateway 启动和回滚步骤。P5-G 仍然是草案和 dry-run，不接飞书。

本阶段依据：

- P5-F 已定义个人飞书凭证与授权方案：env SecretRef、个人 DM allowlist、groups disabled、dynamic agent creation disabled、streaming disabled。
- `openclaw config set --help` 支持 SecretRef builder：`--ref-source env --ref-provider default --ref-id <ENV>`，并支持 `--dry-run`。
- `openclaw config patch --help` 支持从 JSON5 patch 文件执行 validated write，且支持 `--dry-run --json`。
- `openclaw config validate --help` 支持 `--json`。
- `openclaw channels status --help` 支持 `--channel feishu`、`--json`、`--probe`。
- `openclaw channels remove --help` 支持 `--channel feishu`、`--account` 和 `--delete`。
- `openclaw gateway --help` 显示 gateway 可前台 `run`，并支持 `--bind loopback`。

本阶段边界：

- 不写 `/private/tmp/openclaw-personal-feishu.patch.json5`。
- 不执行 `openclaw config patch --file ...` 的真实写入。
- 不执行 `openclaw channels login --channel feishu`。
- 不执行 `openclaw channels status --channel feishu --probe`。
- 不执行 `openclaw gateway run --bind loopback`。
- 不读取真实 app id / app secret / verification token / encrypt key。
- 不写 `~/.openclaw` 配置，不启动 gateway，不安装 daemon / LaunchAgent，不发送飞书消息，不调用真实 `codex exec`。
- 不复用公司 `cc-connect` / Feishu，不使用公司 app、tenant、群组、联系人或历史消息。

脚本：

- 路径：`scripts/openclaw_feishu_channel_config_draft.py`
- 依赖：复用 `scripts/openclaw_feishu_auth_dry_run.py` 的 P5-F 凭证与授权方案。
- 输入：`--credential-source`、`--app-id-env`、`--app-secret-env`、`--allow-user`、`--dm-policy`、`--group-policy`、`--account`、`--name`、`--domain`、`--connection-mode`、`--patch-path`。
- 默认 patch 路径预览：`/private/tmp/openclaw-personal-feishu.patch.json5`。
- 输出：JSON 配置草案，包括 P5-F auth plan、config patch JSON5 shape、preflight 命令、patch dry-run 命令、未来真实写入命令、post-config 本地验收、credential probe 预览、gateway 启动预览、消息测试预览和 rollback 预览。

未来配置顺序草案：

1. preflight：只读确认 active config 路径、当前 config validate、gateway 状态和 Feishu channel 本地状态。
2. patch draft：用户确认后，才在受控 shell 里创建 `/private/tmp/openclaw-personal-feishu.patch.json5`；P5-G 不创建该文件。
3. patch dry-run：先执行 `openclaw config patch --file /private/tmp/openclaw-personal-feishu.patch.json5 --dry-run --json`，只验证 schema / SecretRef 形态。
4. config apply：只有 P5-H 逐项审批后，才允许执行真实 `openclaw config patch --file ...`。
5. local verify：真实配置后，先执行 `openclaw config validate --json` 和 `openclaw channels status --channel feishu --json`，不 probe。
6. probe：如需联系 Feishu provider 验证凭证，`--probe` 必须再次单独确认。
7. gateway：如需启动 gateway，先只允许前台 `openclaw gateway run --bind loopback`，不得安装 daemon / LaunchAgent。
8. message test：如需发/收一条个人 DM 测试消息，必须单独确认。
9. rollback：优先 soft remove；`--delete` 删除配置项必须再次确认。

预演命令：

```bash
python3 -B scripts/openclaw_feishu_channel_config_draft.py --examples
python3 -B scripts/openclaw_feishu_channel_config_draft.py --allow-user ou_personal_open_id_placeholder
python3 -B scripts/openclaw_feishu_channel_config_draft.py --allow-user ou_personal_open_id_placeholder --group-policy allowlist --allow-group oc_group_placeholder
...
- env SecretRef + 个人 open_id placeholder 示例返回 `status=draft_ready`，但 `configuration_allowed_now=false`、`may_read_credentials=false`、`may_write_openclaw_config=false`、`may_probe_provider=false`。
- 缺少 allowlist 示例返回 `status=hold`，不生成配置序列。
- inline credential 示例返回 `status=hold`，不生成配置序列。
- 尝试启用 group allowlist 示例返回 `status=hold`，不生成配置序列。
- 编译检查在临时缓存目录下通过。
- 所有命令都只是 command preview，`execution_allowed_now=false`。

未来 patch dry-run 预览：

```bash
openclaw config patch --file /private/tmp/openclaw-personal-feishu.patch.json5 --dry-run --json
```
...
```bash
openclaw config patch --file /private/tmp/openclaw-personal-feishu.patch.json5
openclaw channels status --channel feishu --json --probe
openclaw gateway run --bind loopback
openclaw channels remove --channel feishu --account personal-feishu-bot --delete
```

结论：P5-G 已完成最小个人飞书 channel 配置草案。下一步不能直接配置飞书；应进入 P5-H 真实个人飞书 channel 配置审批清单，逐项确认凭证输入、个人 open_id、patch 文件创建、dry-run 验收、是否允许真实写入、是否允许 probe、是否允许启动 gateway、是否允许消息测试和回滚策略。


### P5-H 真实个人飞书 channel 配置审批清单（2026-06-25）

目标：在任何真实个人飞书 channel 配置前，建立逐项审批门禁。P5-H 只写审批清单，不执行真实配置，不创建 patch 文件，不读取 app id / app secret，不连接飞书。

P5-H 必须确认的审批项：

| 审批项 | 必须确认的内容 | 默认建议 | 未确认时状态 |
| --- | --- | --- | --- |
| 是否进入真实配置 | 是否允许未来执行 `openclaw config patch --file /private/tmp/openclaw-personal-feishu.patch.json5` | 暂不执行，先审阅清单 | block |
| 凭证输入方式 | 个人飞书 app id / app secret 如何进入运行环境 | 仅使用 `OPENCLAW_FEISHU_APP_ID` 和 `OPENCLAW_FEISHU_APP_SECRET` 环境变量，由用户现场输入 | block |
| 凭证记录边界 | 是否允许 Codex、手册、日志、Git 或命令历史记录真实凭证 | 不允许记录、复述、读取或保存任何真实 secret | block |
| 个人 open_id | 允许哪个个人飞书 open_id 触发 bot | 只允许用户本人；open_id 必须来自后续单独批准的个人配对或本地查询步骤 | block |
| 公司边界 | 是否确认不使用公司 app、tenant、联系人、群组、历史消息或旧 `cc-connect` 配置 | 必须确认个人专用，不复用公司通道 | block |
| patch 文件创建 | 是否允许创建 `/private/tmp/openclaw-personal-feishu.patch.json5` | 只在临时路径创建 reviewed SecretRef patch，不包含真实 secret | block |
| patch dry-run | 是否允许执行 `openclaw config patch --file ... --dry-run --json` | 先 dry-run，只验证 schema 和 SecretRef 形态 | block |
| 真实配置写入 | dry-run 通过后是否允许真实写入 OpenClaw config | 默认仍不写入；必须单独明确确认 | block |
| 本地验收范围 | 真实写入后先做哪些检查 | 只做 `openclaw config validate --json` 和 `openclaw channels status --channel feishu --json`，不 probe | block |
| provider probe | 是否允许执行 `openclaw channels status --channel feishu --json --probe` | 默认不允许；需要单独确认，因为会联系 Feishu provider | block |
| gateway 启动 | 是否允许启动 OpenClaw gateway | 默认不启动；如批准，只允许前台 `openclaw gateway run --bind loopback` | block |
| 消息测试 | 是否允许发送或接收一条个人飞书 DM 测试消息 | 默认不发送；必须单独确认测试内容与验收标准 | block |
| 回滚方式 | 配置不符合预期时如何撤回 | 优先 soft remove；`--delete` 删除配置必须再次确认 | block |

允许动作（仅在 P5-H 对应审批项逐项确认后，且进入后续阶段时才可执行）：

- 只读 preflight：`openclaw config file`。
- 只读 preflight：`openclaw config validate --json`。
- 只读 preflight：`openclaw gateway status --deep --no-probe --json`。
- 只读 preflight：`openclaw channels status --channel feishu --json`。
- 在用户确认后创建 `/private/tmp/openclaw-personal-feishu.patch.json5`，且 patch 内容只能包含 env SecretRef。
- 在用户确认后执行 patch dry-run：`openclaw config patch --file /private/tmp/openclaw-personal-feishu.patch.json5 --dry-run --json`。
- 在 dry-run 通过且用户再次确认后执行真实 config patch。
- 配置后只读检查：`openclaw config validate --json`。
- 配置后只读检查：`openclaw channels status --channel feishu --json`。
- 必要时执行 soft rollback：`openclaw channels remove --channel feishu --account personal-feishu-bot`。

禁止动作（P5-H 不允许，后续也必须单独审批）：

- 不在聊天记录、手册、日志、Git、命令历史或 patch 文件中写入真实 app id / app secret / verification token / encrypt key。
- 不让 Codex 读取、打印、复述或保存真实飞书凭证。
- 不使用 inline credential 命令。
- 不创建包含真实 secret 的 OpenClaw config。
- 不复用公司 `cc-connect` / Feishu，不恢复旧公司运行目录、会话、日志、LaunchAgent 或同步脚本。
- 不使用公司 app、tenant、联系人、群组或历史消息。
- 不启用群聊，不配置 group allowlist。
- 不执行 provider probe，除非单独确认。
- 不启动 gateway，除非单独确认。
- 不安装 daemon / LaunchAgent。
- 不发送飞书测试消息，除非单独确认。
- 不调用真实 `codex exec`。
- 不执行 `openclaw channels remove --channel feishu --account personal-feishu-bot --delete`，除非单独确认。

建议的真实配置窗口：

- 在 tmux `gateway` 或临时受控 shell 中执行。
- 开始前确认 Git 工作区干净。
- 开始前记录只读 preflight 摘要，但不记录任何 secret。
- 真实 app id / app secret 只由用户在本地 shell 输入到环境变量，不由 Codex 打印、读取、保存或复述。
- patch 文件只使用 SecretRef 引用 `OPENCLAW_FEISHU_APP_ID` 和 `OPENCLAW_FEISHU_APP_SECRET`。
- patch dry-run 通过前不得真实写入配置。
- 本地验收通过前不得 probe、不得启动 gateway、不得测试消息。
- 执行完成后关闭该 shell，或清理当前 shell 中的飞书凭证环境变量。

P5-I 进入条件：

1. 用户明确说“进入 P5-I 真实个人飞书 channel 配置执行预备检查”，或明确确认 P5-H 全部审批项。
2. 用户确认凭证输入方式为 env SecretRef，并确认真实凭证不会写入手册、日志、Git 或聊天记录。
3. 用户确认个人 open_id 来源为个人飞书，不来自公司通讯录、公司 tenant 或旧 `cc-connect`。
4. 用户确认初始范围只允许个人 DM allowlist，群聊保持 disabled。
5. 用户确认是否允许创建临时 patch 文件。
6. 用户确认是否允许执行 patch dry-run。
7. 用户确认 dry-run 通过后是否允许真实 config patch；如果未确认，真实写入继续 block。
8. 用户确认 provider probe、gateway 启动和消息测试是否分别保持 block 或单独放行。
9. 用户确认回滚策略先 soft remove，删除配置需再次确认。

结论：P5-H 是真实个人飞书配置前的审批闸门。清单未逐项确认前，不得创建真实 patch 文件，不得写 OpenClaw config，不得 probe，不得启动 gateway，不得发送消息。


### P5-I 真实个人飞书 channel 配置执行预备检查（2026-06-25）

目标：在真实个人飞书配置前，执行一轮只读 preflight，确认本地 OpenClaw 基线状态、配置文件状态、gateway 状态、Feishu channel 状态和凭证输入边界。P5-I 不创建 patch 文件，不执行 patch dry-run，不写真实配置，不读取飞书凭证，不连接飞书。

本阶段执行的只读检查：

| 检查项 | 命令或方法 | 结果摘要 |
| --- | --- | --- |
| OpenClaw 版本 | `openclaw --version` | `OpenClaw 2026.6.8 (844f405)` |
| active config 路径 | `openclaw config file` | `~/.openclaw/openclaw.json` |
| config validate | `openclaw config validate --json` | `valid=true`，路径为 `/Users/duckulacissy/.openclaw/openclaw.json` |
| gateway 状态 | `openclaw gateway status --deep --no-probe --json` | LaunchAgent 未加载；config audit ok；bind mode 为 loopback；端口 `18789` 空闲；无监听者 |
| Feishu channel 本地状态 | `openclaw channels status --channel feishu --json` | `configuredChannels=[]`；gateway 不可达原因是 gateway auth secret 在当前命令路径不可用；`configOnly=true` |
| 飞书 env 状态 | 只检查变量是否存在，不打印值 | `OPENCLAW_FEISHU_APP_ID=UNSET`，`OPENCLAW_FEISHU_APP_SECRET=UNSET` |
| patch 文件状态 | 检查 `/private/tmp/openclaw-personal-feishu.patch.json5` 是否存在 | 不存在 |

沙盒备注：

- 初次在 Codex 沙盒内执行 `openclaw config file` 和 `openclaw channels status --channel feishu --json` 时，OpenClaw 因无法打开本地状态库返回 `unable to open database file`。
- 随后在用户批准的本机权限下重跑同一只读 preflight，命令成功。
- 该问题来自本地 `~/.openclaw/state/openclaw.sqlite` 的权限访问路径，不是 Feishu 连接，也不是配置写入。

P5-I 结论：

- 本地 OpenClaw config 文件存在且有效。
- gateway 未作为 LaunchAgent 运行，端口 `18789` 空闲。
- 当前没有配置 Feishu channel。
- 飞书 app id / app secret 环境变量当前未设置。
- 临时 patch 文件不存在。
- 可以进入下一步“引导用户获取个人飞书 app 与个人 open_id”的准备，但不能直接写配置。

仍然禁止：

- 不创建 `/private/tmp/openclaw-personal-feishu.patch.json5`。
- 不执行 `openclaw config patch --file ... --dry-run --json`。
- 不执行真实 `openclaw config patch`。
- 不执行 `openclaw channels status --channel feishu --json --probe`。
- 不读取、打印、保存或复述真实 app id / app secret / verification token / encrypt key。
- 不启动 gateway，不安装 daemon / LaunchAgent，不发送飞书消息，不调用真实 `codex exec`。
- 不复用公司 `cc-connect` / Feishu。

下一步建议：进入 P5-J 个人飞书 app 与 open_id 获取引导。P5-J 应只指导用户在个人飞书侧创建/确认应用、权限和 open_id 获取路径；真实 secret 仍由用户本地保存，不交给 Codex，不写入手册、日志或 Git。


### P5-J/P5-K 个人飞书真实接入、Codex 中转模型验证与 LaunchAgent 常驻（2026-06-25）

目标：在用户逐项批准后，完成个人飞书最小真实入口配置，并把 OpenClaw 的模型调用路线对齐到本地 Codex 当前使用的中转 API 配置；随后把 gateway 从临时前台运行切换为可观测的本机 LaunchAgent 常驻服务。

本阶段已完成：

| 项目 | 结果 |
| --- | --- |
| 个人飞书机器人 | 复用既有个人飞书机器人，使用 bot 身份接入 |
| 入口范围 | 仅个人 DM allowlist；群聊保持 disabled |
| 事件连接 | 使用 Feishu channel WebSocket 模式 |
| 公司边界 | 不复用公司 `cc-connect` / Feishu，不恢复旧公司运行目录、会话、日志、LaunchAgent 或同步脚本 |
| 本地 Codex API 路由 | 本地 Codex 使用 `xbai` 中转配置：`model_provider="xbai"`，`model="gpt-5.5"`，`base_url="https://api.xbai.top/v1"`，`wire_api="responses"` |
| OpenClaw 模型路由 | 已对齐为 `xbai/gpt-5.5`，provider `baseUrl` 为 `https://api.xbai.top/v1`，API 类型为 `openai-responses` |
| 密钥处理 | API key 仅在本地 Codex / OpenClaw auth store 内部保存和引用；手册、日志、Git 和聊天中均不记录密钥值 |
| 实测结果 | 个人飞书 DM 触发模型请求成功，HTTP `200`，Feishu dispatch 完成 `replies=1` |
| 常驻服务 | Gateway 已安装为 macOS LaunchAgent 并运行；RPC 可达，端口 `18789` loopback 监听，Feishu probe 通过 |

关键结论：

- 本地 Codex API 不是直连官方 OpenAI endpoint，而是使用 `xbai` 中转配置；OpenClaw 已按这一点完成模型 provider 对齐。
- OpenClaw 调用模型时使用 `openai-responses` 协议形态，与本地 Codex 的 `wire_api="responses"` 一致。
- 当前个人飞书入口已具备最小可用闭环：用户 DM → 飞书 bot → OpenClaw gateway → `xbai/gpt-5.5` 中转模型 → 飞书回复。
- 本阶段没有把任何真实 app secret、API key、verification token 或 encrypt key 写入手册、日志或 Git。
...
- 不扩大个人 DM allowlist。
- 不复用公司 `cc-connect` / Feishu。
- 不把任何 secret 值写入仓库、手册、日志或聊天。
- 不切换模型 provider、模型名、base URL 或协议形态，除非再次获得用户明确确认。
- 不开放公网端口；gateway 仍保持 loopback / 本机服务边界。

下一步建议：进入 P5-L 部署收尾。P5-L 应完成 Git 变更审查、必要提交与推送、LaunchAgent 健康检查 SOP、日志轮转/回滚说明，并观察真实个人飞书入口在短周期内是否稳定。


### P5-L 部署收尾：Git 审查、运行观测与回滚 SOP（2026-06-25）

目标：在个人飞书最小入口和中转模型链路跑通后，完成部署收尾：审查 Git 变更、建立 gateway 运行与回滚 SOP、复核只读健康状态，并把当前边界写入仓库。

本阶段已完成：

| 项目 | 结果 |
| --- | --- |
| Git diff 审查 | 当前变更为文档和日志更新；未发现真实密钥值写入 |
| Secret 扫描 | 未发现 `sk-...`、Feishu app secret、OpenAI key 或 token 值形态 |
| Gateway SOP | 新增 `docs/OPENCLAW_GATEWAY_SOP.md`，记录健康检查、日志检查、审批边界、软回滚和硬回滚 |
| 仓库入口校准 | `README.md` 和 `AGENTS.md` 当前阶段更新为 P5-L |
| 执行 SOP 校准 | `docs/AI_EXECUTION_SOP.md` 补充当前个人飞书 DM allowlist 与 `xbai` 中转模型路由边界 |
| Gateway 健康检查 | LaunchAgent loaded/running，RPC ok，版本 `2026.6.8`，端口 `18789` 仅在 `127.0.0.1` / `[::1]` 监听 |
| Feishu probe | Feishu channel configured/running/probe ok |
| 模型路由 | `agents.defaults.model.primary` 仍为 `xbai/gpt-5.5` |

P5-L 边界：

- 本阶段只做只读健康检查、文档补齐、Git 审查和 SOP 记录。
- 未停止、重启、卸载或修改 LaunchAgent。
- 未改 Feishu app、DM allowlist、group policy、模型 provider、base URL 或 auth profile。
- 未读取、打印或写入任何真实 secret。
- 未启用群聊，未扩大入口范围，未开放公网端口。

结论：P5-L 部署收尾通过。个人飞书最小入口当前可观测、可回滚、可审计；后续应先做短周期入口观察与真实任务边界验收，再决定是否进入更复杂的 OpenClaw 正式任务转发。

下一步建议：进入 P5-M 短周期入口观察与真实任务边界验收。P5-M 应只选择低风险、可回滚、无密钥的真实任务进行观察，不扩大入口范围，不启用群聊，不改模型路由。


### P5-MNO 合并验收：入口观察、任务转发边界与 P5 正式收口（2026-06-25）

目标：按用户要求把剩余 P5-M / P5-N / P5-O 三个阶段合并一次完成：不再继续细分；完成入口观察、任务转发规则、P5 正式验收和日常运维 SOP。P6 NAS 知识库仅保留位置，暂不接入。

本阶段已完成：

| 项目 | 结果 |
| --- | --- |
| 入口观察 | 个人飞书入口保持 configured/running/probe ok；gateway LaunchAgent loaded/running；RPC ok |
| 模型路由 | `agents.defaults.model.primary` 仍为 `xbai/gpt-5.5`；本地 Codex / OpenClaw 继续使用中转路线 |
| 端口边界 | gateway 仍只监听 `127.0.0.1` 和 `[::1]` 的 `18789`，未开放公网 |
| 任务转发边界 | 明确 L0/L1/L2/L3 风险分级、审批状态、任务包格式和 Codex 返回格式 |
| 日常运维 SOP | 新增 `docs/DAILY_OPERATIONS_SOP.md`，覆盖每日健康检查、入口规则、任务转发、日志/Git、安全边界、故障处理、回滚和周期维护 |
| P6 NAS | 只保留位置；未挂载 NAS、未配置路径、未处理凭据、未启用索引或检索 |
| P5 验收 | P5 正式部署收口：个人飞书入口可用、模型链路可用、边界明确、SOP 完整、Git 可追溯 |

日常运维内容：

- 每日健康检查：gateway、LaunchAgent、Feishu channel、端口、模型路由和 Git 工作区。
- 消息入口规则：区分可直接处理的低风险问答、必须确认的变更任务、默认阻断的密钥/公网/公司通道/群聊动作。
- 任务转发规则：使用 `task_id`、来源、仓库、阶段、风险等级、审批状态、请求和预期输出组成任务包。
- 日志与 Git：阶段性操作写 `logs/YYYY-MM-DD.md`，架构/边界变更写手册，SOP 变更写 `docs/*_SOP.md`，通过检查后提交推送。
- 安全边界：不记录 key、token、secret、`.env`、私钥；不扩大 allowlist；不启群聊；不改模型路线；不复用公司通道。
- 故障处理：按 Feishu 不回复、gateway down、模型请求失败、Git dirty 等场景记录检查顺序；重启或改配置前必须确认。
- 回滚策略：软回滚先停服务并保留配置用于审计；硬回滚涉及删除 channel、LaunchAgent、auth/env，必须二次确认。
- 周期维护：定期检查日志大小、版本漂移、SOP 是否同步、Git 是否干净；升级 OpenClaw 或改模型路线必须单独审批。

P5-MNO 边界：

- 未发送新的外部测试消息。
- 未停止、重启、卸载或修改 LaunchAgent。
- 未修改 Feishu app、allowlist、group policy、模型 provider、base URL、API shape 或 auth profile。
- 未读取、打印或写入任何真实 secret。
- 未接入 NAS，未配置 NAS 路径、凭据、索引或检索。

验收结论：P5 OpenClaw 正式部署阶段已完成。当前系统进入 P7 日常运维试运行；P6 NAS 知识库接入保留为后续独立阶段。

下一步建议：进入 P7 日常运维试运行，按 `docs/DAILY_OPERATIONS_SOP.md` 执行每日健康检查和真实低风险任务观察。P6 NAS 在用户明确要求前只保留位置。


### P7 日常运维最终验收（2026-06-25）

目标：按用户要求一次性完成 P7，不再拆分。P7 的范围是把日常运维从“试运行”收口为“常态运维”：确认 SOP 可执行、关键健康检查通过、风险边界有效、日志/Git/手册保持一致。P6 NAS 仍只保留位置，不接入。

P7 最终验收结果：

| 验收项 | 结果 |
| --- | --- |
| Gateway | LaunchAgent loaded/running；OpenClaw `2026.6.8`；RPC ok |
| 端口边界 | `18789` 仅监听 `127.0.0.1` 和 `[::1]`，未开放公网 |
| Feishu channel | configured/running/probe ok |
| 模型路由 | `agents.defaults.model.primary` 仍为 `xbai/gpt-5.5` |
| Git 工作区 | 干净，无未提交变更 |
| 日常运维 SOP | `docs/DAILY_OPERATIONS_SOP.md` 已覆盖每日健康检查、入口规则、任务转发、日志/Git、安全边界、故障处理、回滚、周期维护和 P6 NAS 占位 |
| 密钥边界 | 未读取、打印、移动、修改或记录任何真实 secret |
| NAS | 未挂载、未配置路径、未处理凭据、未启用索引或检索 |
...
验收结论：P7 日常运维阶段已完成。当前系统进入常态日常运维；后续只在用户明确要求时进入 P6 NAS 知识库接入。

下一步建议：保持常态日常运维。日常低风险状态检查可按 `docs/DAILY_OPERATIONS_SOP.md` 执行；任何服务变更、模型路线变更、飞书入口范围变更、密钥处理或 NAS 接入都需要单独确认。
...
日期：2026-06-24。完成事项：完成 P5-G 最小个人飞书 channel 配置草案，并新增 `scripts/openclaw_feishu_channel_config_draft.py`。

依据：只读核对 `openclaw config set --help`、`openclaw config patch --help`、`openclaw config validate --help`、`openclaw channels status --help`、`openclaw channels remove --help` 和 `openclaw gateway --help`；明确未来可用的 config patch dry-run、本地验收、provider probe、foreground gateway 和 rollback 命令形态。

方案：P5-G 复用 P5-F 的 env SecretRef、个人 DM allowlist、groups disabled 和个人专用边界。脚本只有在 P5-F auth plan 返回 `draft_ready` 时才生成配置序列；否则返回 `hold`，不生成配置序列。配置草案包含未来 patch JSON5 shape、preflight、patch dry-run、真实写入候选、本地验收、probe、gateway 启动、消息测试和回滚命令预览。

脚本验证：`scripts/openclaw_feishu_channel_config_draft.py --examples` 覆盖 env SecretRef + allowlist、缺少 allowlist、inline credential、尝试启用 group allowlist 四类场景；allowlist 完整时返回 `draft_ready`，其他场景 hold 且不生成配置序列。额外执行个人 open_id placeholder 和 group allowlist 单独预演，结果符合预期。`PYTHONPYCACHEPREFIX=/private/tmp/openclaw_feishu_config_pycache python3 -m py_compile scripts/openclaw_feishu_channel_config_draft.py` 通过。

边界保持：本次只写草案、日志和 dry-run 脚本；未连接飞书，未读取或写入 app id / app secret / verification token / encrypt key，未写 `/private/tmp/openclaw-personal-feishu.patch.json5`，未执行 `openclaw config patch` 真实写入，未执行 `openclaw channels login/add/status --probe`，未写 `~/.openclaw` 配置，未启动 gateway，未安装 daemon / LaunchAgent，未发送消息，未调用真实 `codex exec`，未复用公司 `cc-connect` / Feishu。

同步操作：正式中文 Word 手册已按当前 Markdown 重新生成，包含 P5-G 最小个人飞书 channel 配置草案和项目日志 #024。

下一步：进入 P5-H 真实个人飞书 channel 配置审批清单；P5-H 仍只写审批门禁，不执行真实配置。

### 项目日志 #025

日期：2026-06-25。完成事项：完成 P5-H 真实个人飞书 channel 配置审批清单。

依据：承接 P5-F 的个人飞书凭证授权 dry-run 和 P5-G 的最小 channel 配置草案，并沿用 P5-D 真实配置前先建立审批门禁的治理方式。

方案：P5-H 把真实个人飞书配置拆成逐项审批项：是否进入真实配置、凭证输入方式、凭证记录边界、个人 open_id、公司边界、patch 文件创建、patch dry-run、真实配置写入、本地验收、provider probe、gateway 启动、消息测试和回滚方式。默认状态全部为 block，必须由用户逐项确认后才可进入下一阶段。

边界保持：本次只写审批清单和日志；未创建 `/private/tmp/openclaw-personal-feishu.patch.json5`，未读取或写入 app id / app secret / verification token / encrypt key，未执行 `openclaw config patch` 真实写入，未执行 `openclaw channels status --channel feishu --json --probe`，未写 `~/.openclaw` 配置，未启动 gateway，未安装 daemon / LaunchAgent，未发送消息，未调用真实 `codex exec`，未复用公司 `cc-connect` / Feishu。

同步操作：正式中文 Word 手册已按当前 Markdown 重新生成，包含 P5-H 真实个人飞书 channel 配置审批清单和项目日志 #025。

下一步：用户审阅并逐项确认 P5-H；确认后进入 P5-I 真实个人飞书 channel 配置执行预备检查。P5-I 仍不得绕过审批直接写真实配置。

### 项目日志 #026

日期：2026-06-25。完成事项：完成 P5-I 真实个人飞书 channel 配置执行预备检查。

依据：用户要求进行下一步；P5-I 承接 P5-H 审批清单，只允许执行只读 preflight，不允许创建 patch、不允许写真实配置、不允许 probe、不允许启动 gateway。

执行结果：确认 OpenClaw 版本为 `2026.6.8 (844f405)`；active config 为 `~/.openclaw/openclaw.json`；`openclaw config validate --json` 返回 `valid=true`；gateway LaunchAgent 未加载，bind mode 为 loopback，端口 `18789` 空闲且无监听；Feishu channel 本地状态显示 `configuredChannels=[]`，`configOnly=true`；`OPENCLAW_FEISHU_APP_ID` 和 `OPENCLAW_FEISHU_APP_SECRET` 均为未设置；`/private/tmp/openclaw-personal-feishu.patch.json5` 不存在。

沙盒备注：初次在 Codex 沙盒内执行部分 OpenClaw preflight 时，因无法打开 `~/.openclaw/state/openclaw.sqlite` 返回 `unable to open database file`；在用户批准的本机权限下重跑同一只读检查后成功。该问题是状态库访问权限噪声，不代表飞书连接或配置写入。

边界保持：本次只做只读预备检查和文档记录；未创建 patch 文件，未执行 patch dry-run，未执行真实 `openclaw config patch`，未执行 provider probe，未读取或写入 app id / app secret / verification token / encrypt key，未写 `~/.openclaw` 配置，未启动 gateway，未安装 daemon / LaunchAgent，未发送消息，未调用真实 `codex exec`，未复用公司 `cc-connect` / Feishu。

同步操作：正式中文 Word 手册已按当前 Markdown 重新生成，包含 P5-I 真实个人飞书 channel 配置执行预备检查和项目日志 #026。

下一步：进入 P5-J 个人飞书 app 与 open_id 获取引导；P5-J 仍不接触真实 secret，不写真实配置。

### 项目日志 #027

日期：2026-06-25。完成事项：完成 P5-J/P5-K 个人飞书真实接入、Codex 中转模型验证与 LaunchAgent 常驻服务配置。

依据：用户逐步确认后，真实入口路线从 Telegram 改为个人飞书，并允许继续完成飞书机器人配置、模型 API 路由对齐、gateway 启动、消息测试和常驻服务收尾。用户特别要求记录：本地 Codex 的 API 使用中转配置。

执行结果：复用既有个人飞书机器人，采用 bot 身份、WebSocket、个人 DM allowlist 和 groups disabled；OpenClaw 模型路由已对齐本地 Codex 配置，使用 `xbai/gpt-5.5`、`openai-responses` 和 `https://api.xbai.top/v1`。本地 Codex API 使用 `xbai` 中转配置，不直连官方 OpenAI endpoint。个人飞书 DM 实测触发模型请求成功，HTTP `200`，飞书 dispatch 完成 `replies=1`。gateway 已安装为 macOS LaunchAgent 并运行，RPC 可达，端口 `18789` loopback 监听，Feishu channel probe 通过。

密钥边界：真实 API key、app secret、verification token 和 encrypt key 均未写入手册、日志、Git 或聊天；相关值只在本地 auth store / env 文件中保存和引用。

公司边界：未复用公司 `cc-connect` / Feishu；未恢复旧公司运行目录、会话、日志、LaunchAgent 或同步脚本；群聊未启用。

同步操作：正式中文 Word 手册已按当前 Markdown 重新生成，包含 P5-J/P5-K 真实个人飞书接入、Codex 中转模型验证、LaunchAgent 常驻服务和项目日志 #027。

下一步：进入 P5-L Git 变更审查、提交推送、运行观测与回滚 SOP；未经确认不得扩大入口范围、启用群聊、改动模型路由或写入任何 secret。

### 项目日志 #028

日期：2026-06-25。完成事项：完成 P5-L 部署收尾，包括 Git 变更审查、OpenClaw Gateway SOP、只读健康检查、README / AGENTS 阶段校准和运行/回滚边界记录。

依据：用户同意进入下一步；P5-L 承接 P5-J/P5-K，目标是在个人飞书入口与 `xbai` 中转模型链路跑通后，先做封账、审查、SOP 和观察准备，而不是继续扩大功能。

执行结果：新增 `docs/OPENCLAW_GATEWAY_SOP.md`；更新 `README.md`、`AGENTS.md` 和 `docs/AI_EXECUTION_SOP.md`，使仓库入口文档与当前 P5-L 状态一致。Git diff 审查确认当前变更为文档/日志/SOP；secret 扫描未发现真实 key、token 或 app secret 值。只读健康检查显示 gateway LaunchAgent loaded/running，RPC ok，OpenClaw 版本 `2026.6.8`，端口 `18789` 仅监听 `127.0.0.1` 和 `[::1]`；Feishu channel configured/running/probe ok；模型路由仍为 `xbai/gpt-5.5`。

边界保持：未停止、重启、卸载或修改 LaunchAgent；未改 Feishu app、DM allowlist、group policy、模型 provider、base URL 或 auth profile；未读取或记录任何真实 secret；未启用群聊，未扩大入口范围，未开放公网端口，未复用公司 `cc-connect` / Feishu。

同步操作：正式中文 Word 手册已按当前 Markdown 重新生成，包含 P5-L 部署收尾、运行与回滚 SOP、只读健康检查结果和项目日志 #028。

下一步：进入 P5-M 短周期入口观察与真实任务边界验收；只选择低风险、可回滚、无密钥的真实任务进行观察，不扩大入口范围，不启用群聊，不改模型路由。

### 项目日志 #029

日期：2026-06-25。完成事项：按用户要求把 P5-M / P5-N / P5-O 三个阶段整合为 P5-MNO 一次完成，并新增日常运维 SOP。

依据：用户要求“三个阶段整合，一次做完。NAS 暂时不添加，位置留出。介绍一下日常运维的内容”，随后确认继续。

执行结果：新增 `docs/DAILY_OPERATIONS_SOP.md`，明确每日健康检查、消息入口规则、任务转发规则、日志与 Git、安全边界、故障处理、回滚策略和周期维护。更新 `README.md`、`AGENTS.md`、`docs/AI_EXECUTION_SOP.md`、`docs/OPENCLAW_GATEWAY_SOP.md` 和本手册，使当前状态从 P5-L 收尾推进为 P5-MNO 正式验收完成。只读健康检查显示 gateway LaunchAgent loaded/running，RPC ok，OpenClaw 版本 `2026.6.8`，端口 `18789` 仅监听 `127.0.0.1` 和 `[::1]`；Feishu channel configured/running/probe ok；模型路由仍为 `xbai/gpt-5.5`。

NAS 边界：P6 NAS 知识库接入只保留位置；本次未挂载 NAS、未配置路径、未处理凭据、未启用索引或检索、未迁移记忆资产。

安全边界：未发送新的外部测试消息；未停止、重启、卸载或修改 LaunchAgent；未改 Feishu app、allowlist、group policy、模型 provider、base URL、API shape 或 auth profile；未读取或记录任何真实 secret；未启用群聊、未扩大入口范围、未开放公网端口、未复用公司 `cc-connect` / Feishu。

同步操作：正式中文 Word 手册已按当前 Markdown 重新生成，包含 P5-MNO 合并验收、日常运维 SOP、P6 NAS 占位和项目日志 #029。

下一步：进入 P7 日常运维试运行；P6 NAS 在用户明确要求前只保留位置。

### 项目日志 #030

日期：2026-06-25。完成事项：按用户要求一次性完成 P7 日常运维最终验收。

依据：用户明确要求“请你一次性完成 P7”。P7 范围限定为日常运维收口：只读健康验收、SOP 状态校准、手册/日志记录和 Git 追溯；不发送外部消息，不改配置，不接入 NAS。

执行结果：复核 `docs/DAILY_OPERATIONS_SOP.md` 的完成标准，并执行只读健康检查。结果显示 gateway LaunchAgent loaded/running，RPC ok，OpenClaw 版本 `2026.6.8`，端口 `18789` 仅监听 `127.0.0.1` 和 `[::1]`；Feishu channel configured/running/probe ok；模型路由仍为 `xbai/gpt-5.5`；Git 工作区干净。同步更新 `README.md`、`AGENTS.md`、`docs/DAILY_OPERATIONS_SOP.md` 和本手册，将状态从 P7 daily operations trial 改为 normal daily operations。

边界保持：未发送新的外部测试消息；未停止、重启、卸载或修改 LaunchAgent；未改 Feishu app、allowlist、group policy、模型 provider、base URL、API shape 或 auth profile；未读取或记录任何真实 secret；未启用群聊、未扩大入口范围、未开放公网端口；未接入 NAS。

同步操作：正式中文 Word 手册已按当前 Markdown 重新生成，包含 P7 日常运维最终验收和项目日志 #030。

下一步：进入常态日常运维；P6 NAS 在用户明确要求前只保留位置。
