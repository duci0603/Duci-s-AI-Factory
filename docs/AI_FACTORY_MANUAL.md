...
- 公司侧 `cc-connect` / Feishu 通道已隔离到本地 `公司使用通道` 目录
...
- P4 个人 gateway 需要重新设计，不复用公司侧 `cc-connect` / Feishu 通道
- NAS 长期记忆库路径与资料分类待补齐
...
| P4 | 个人 gateway 试部署 | 新建个人专用 gateway 配置；不复用公司侧 `cc-connect`；可接收测试消息，可调用本地脚本，不自动执行高风险任务。 |
...
当前已确认：公司侧 `cc-connect` / Feishu 通道不是个人 AI 工厂主流程的一部分，已移动到本地隔离目录。Codex 本体不依赖 `cc-connect`，它通过 Codex App、Codex CLI、tmux `ai-factory` 和 GitHub 继续作为个人 AI 工厂执行层。
...
个人消息入口（待新建）
↓
个人 gateway（待选型，不复用公司侧 cc-connect）
↓
任务判断与权限检查
↓
